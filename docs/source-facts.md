...
Declaration order and definition order are separate constraints. When a class-body
inline would move against its recorded header source-line stream, retain its
declaration in the class and define it `inline` in the owning header, in source-line
order. Preserve virtual order, field offsets, default arguments on declarations,
and any `@dc-inline-origin` annotation on the relocated definition. A declaration
reordering alone does not authorize moving the implementation to another header.

The access/ownership integration checked 648 class snapshots without changing
sizes, fields, virtual order, or non-friend interfaces. Specific callback
friendships retain their owning retail/DC evidence. The three declaration-only
members `CSpriteFrame::GetWidth`, `CSpriteFrame::GetHeight`, and
`resource::GetReferenceCount` have no recovered procedure or foreign-header inline
location in the pinned symbols; their declarations do not authorize invented body
locations. This remains a coverage gap. The resource disposal paths can consume
the canonical `Release` result directly; the sprite pointer-drawing caller still
has unresolved width/height helper bodies and is not an exact implementation.
