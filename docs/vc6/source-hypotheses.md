...
`homm3 vc6 hypotheses` adapts King's Field's `kf hypotheses` manifest and
...
```sh
homm3 vc6 hypotheses /tmp/reference.json -j 8 --keep-top 10
```
...
Source, ledger, headers, compiler and scoring inputs must stay unchanged during
