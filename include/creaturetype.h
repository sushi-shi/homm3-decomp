...
#define isRetailAcceptedCreatureType(type)                                                         \
    ((type) >= CREATURE_ROSTER_BEGIN && (type) <= CREATURE_RETAIL_RANGE_MAX)
...
{
    if (type < CREATURE_ROSTER_BEGIN || type > CREATURE_RETAIL_RANGE_MAX) {
