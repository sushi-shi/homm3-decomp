...
public:
    ...
    Bitmap816* m_textBack;
         // 0x50, ResourceManager::GetBitmap816
    CTextEntrySave* m_saveBack;
    // 0x54
    unsigned short m_cursorIndex;
  // 0x58, = Text.size() after every edit
    unsigned short m_maxLength;
    // 0x5a, the ctor's textStringSize
    short m_boxWidth;
              // 0x5c, the inset text box
    short m_boxHeight;
             // 0x5e
    short m_boxX;
                  // 0x60
    ...
    short m_textLines;
              // 0x64, ctor stores 1. OnKeyPress
                                 ...
                                 // writes it.
    short m_attributes;
              // 0x66, ctor stores the inset flag
    short m_type;
              // 0x68, compared against 3 by Draw /
    ...
    short m_displayStart;
          // 0x6a, first shown character
    unsigned char m_cursorFlashOn;
      // 0x6c, the caret blink phase: OnKeyPress
    ...
    unsigned char m_hasFocus;
     // 0x6d, stored by SetFocus 0x5bab50
    unsigned char m_autoDraw;
     // 0x6e, gates SetFocus's redraw
...
protected:
    virtual void saveBackground() const;
         // slot 18, retail 0x5bba70
};
