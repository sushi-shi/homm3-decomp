...
public:
    ...
    long getVisitedCount() const;
    pathCell* getVisitedCell(long index);
    ...
    unsigned char isMoat(short hex) const;
...
// findpath.h:194 (Dreamcast source body).
VA(0x004b3b90, 0x20)  // caller/get_hex correlation, dc 0x27fe8
inline     pathCell* searchArray::getHex(long x) const
    {
        if (m_cellData == 0)
            return 0;
        return &m_cellData[x];
...
// findpath.h:202 (Dreamcast source body).
VA(0x0042ecc0, 0x62)  // hd-crossbuild + exact body/callers x2, dc 0x20064
inline     pathCell* searchArray::getCell(type_point point, bool flying) const
    {
        if (!m_cellData)
            return m_cellData;
...
// findpath.h:211 (Dreamcast source body).
inline void searchArray::clearPath()
    {
        m_result.erase(m_result.begin(), m_result.end());
    }

// findpath.h:216 (Dreamcast source body).
inline long searchArray::getPathSteps() const
    {
        return m_result.size();
    }

// findpath.h:221 (Dreamcast source body).
inline unsigned char searchArray::getStep(long i) const
    {
        return m_result[i]->m_direction;
    }

// findpath.h:226 (Dreamcast source body).
inline const pathCell* searchArray::getStepCell(long i) const
    {
        return m_result[i];
    }

// findpath.h:231 (Dreamcast source body).
inline long searchArray::getVisitedCount() const { return m_visitedPoints.size(); }

// findpath.h:236 (Dreamcast source body).
inline pathCell* searchArray::getVisitedCell(long index) { return m_visitedPoints[index]; }

// findpath.h:242 (Dreamcast source body).
inline unsigned char searchArray::isMoat(short hex) const { return m_isMoatSlowed[hex]; }

// findpath.h:247 (Dreamcast source body).
inline bool searchArray::limitWasReached() const { return m_limitReached != 0; }

// findpath.h:252 (Dreamcast source body).
inline void searchArray::setDangerZones(long* dangerZoneMap)
    {
        m_dangerZones = dangerZoneMap;
    }

// findpath.h:257 (Dreamcast source body).
inline void searchArray::setRectangle(tagRECT& rect)
    {
        m_validRectangle = rect;
    }

// findpath.h:265 (Dreamcast source body).
...
// findpath.h:270 (Dreamcast source body).
