...
// winmgr.h:193 (Dreamcast source body).
inline void heroWindowManager::fizzleForwardX(const SLimitData& limits, int fadeTime)
    {
        fizzleForwardX(limits.m_minX, limits.m_minY,
                       limits.width(), limits.height(), fadeTime);
    }

// winmgr.h:198 (Dreamcast source body).
inline void heroWindowManager::saveFizzleSourceX(const SLimitData& limits)
    {
