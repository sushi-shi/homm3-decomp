...
Inspect the Mac callee and callers before treating a retained call as a game helper; library, runtime, glue, and generated functions need separate handling. An identified game helper retained as a Mac call, or visibly expanded in a Mac caller, is sufficient to restore its operation in corresponding Windows source callers, including callers already exact. Do not gate that edit on an original name, an exact Mac byte comparison, or an immediate Windows score gain. Keep a recovered helper even if its project name is provisional; rename it when better evidence appears instead of expanding it back into callers. A Mac call that is already represented by a nested source helper needs no duplicate body.
