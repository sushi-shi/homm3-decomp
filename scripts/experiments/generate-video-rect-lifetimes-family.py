...
geometry = "    POINT pt;\n    RECT src;\n    DDSURFACEDESC ddsd;\n"
destination = "            RECT dst;\n"
geometry_lines = ("POINT pt;", "RECT dst;", "RECT src;", "DDSURFACEDESC ddsd;")
assert all(body.count(x) == 1 for x in (smack_head, bink_head, geometry, destination))
...
for smk_kind,bnk_kind,geometry_scope in itertools.product(range(3),range(3),range(4)):
    ...
    c=capture_loop(c,bnk_kind,bink_head,'            g_windowManager->updateScreen(BinkManager::s_playingBink.m_x + x',[
    ...
    if geometry_scope:
        c = c.replace(geometry, '').replace(destination, '')
        if geometry_scope == 1:
            declarations = ''.join('    ' + line + '\n' for line in geometry_lines)
            ...
            for initialization, declaration in (
                ('pt.x = 0;', 'POINT pt;'),
                ('dst.left = 0;', 'RECT dst;'),
                ('src.left = 0;', 'RECT src;'),
                ('memset(&ddsd, 0, sizeof(ddsd));', 'DDSURFACEDESC ddsd;'),
            ):
                c = c.replace('            ' + initialization,
                              '            ' + declaration + '\n            ' + initialization, 1)
...
a.output.write_text(json.dumps({
    ...
    'evidence':[
        ...
        'Retail reserves0x9c stack bytes; current0x94. POINT/RECT/RECT/DDSURFACEDESC have real address-taking lifetimes in the overlay arm. Test the current mixed lifetime against declarations in the owning Bink scope, whole function, or at each initialization without changing APIs, object types, or initialization order.',
