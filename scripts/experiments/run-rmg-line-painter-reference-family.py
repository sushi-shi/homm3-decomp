#!/usr/bin/env python3
"""Compare one pointer/reference painter signature at the same fastcall ABI.

Only the exact refresh symbol name is aliased in disposable comparison views.
Raw objects, all section bytes and relocation records remain unchanged.
The source types remain distinct; this is not a production normalizer change.
"""
import json
import csv
import hashlib
...
REFERENCE = '?refreshRmgLinePoint@@YIXAAVTRmgLinePainterInterface@@ABU?$TRmgCoordinatePoint@I@@@Z'
VALUE = '?refreshRmgLinePoint@@YIXPAVTRmgLinePainterInterface@@ABU?$TRmgCoordinatePoint@I@@@Z'
...
def main():
    ...
    baseline = CoffObject((root / 'build/objdiff/base/rmg_terrain.obj').read_bytes())
