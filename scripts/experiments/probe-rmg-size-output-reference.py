#!/usr/bin/env python3
"""Tiny VC6 explicit output-reference versus hidden return-buffer diagnostic.
...
def main():
    ...
    original = prefix + '\n' + body + '\n'
    results = []
    for output_reference in (False, True):
        text = original
        if output_reference:
            for owner in ('TRmgMapInterface', 'type_random_map'):
                ...
                text = text[:start] + group.replace('virtual TRmgGridPoint getSize()',
                    'virtual TRmgGridPoint& getSize(TRmgGridPoint& output)') + text[end:]
        ...
        state = 'output_reference' if output_reference else 'value_control'
