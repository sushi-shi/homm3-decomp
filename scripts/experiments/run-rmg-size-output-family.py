#!/usr/bin/env python3
"""Run the size output-reference family with one audited ABI claim alias.
...
The map getSize hidden-result ABI and explicit output-reference ABI share one
retail body claim; only its exact mangled name is mapped for comparison.
This does not claim their C++ function types are identical.
"""
import json
import csv
import hashlib
...
REFERENCE = '?getSize@type_random_map@@UAEAAU?$TRmgCoordinatePoint@I@@AAU2@@Z'
VALUE = '?getSize@type_random_map@@UAE?AU?$TRmgCoordinatePoint@I@@XZ'
