...
"""homm3.match.status - the objdiff scoreboard and checkpoint ledger.
  ...
  homm3 status check     print every function below its recorded best;
                         this is observational and never gates a build
...
is retired because it contains no historical match evidence. A lower current
score is a checkpoint dip, not evidence that the source change is wrong: the
Dreamcast/source/retail gates decide admissibility, while this ledger preserves
both the current frontier and prior peaks. The report is regenerated fresh each
call (`objdiff-cli report generate`, sub-second at this unit count; homm2's
...
def update_rows(current: dict, previous: dict,
                rvas: dict) -> tuple[dict, dict]:
...
def checkpoint_dips(current: dict, rows: dict) -> list[tuple]:
    """Current rows below their high-water checkpoint, in stable order."""
    dips = []
    for key, row in sorted(rows.items()):
        best = row.max
        value = current.get(key)
        if value is None:
            dips.append((key, best, None))
        elif value < best - EPS:
            dips.append((key, best, value))
    return dips
...
def cmd_check(report: dict) -> int:
    ...
    dips = checkpoint_dips(current, rows)
    for (unit, fn), best, value in dips:
        now = f"{value:.2f}%" if value is not None else "MISSING"
        print(f"[status] CHECKPOINT DIP {unit} {fn}: "
              f"high-water {best:.2f}% -> {now}")
    if dips:
        print(f"[status] {len(dips)} function(s) below a prior score "
              "checkpoint. This is observational: keep the peak, resolve "
              "evidence/source gates, and continue coherent reconstruction "
              "through expected dips.")
        return 0
    print(f"[status] checkpoint high-water clean ({len(rows)} functions "
          "at or above their recorded peak)")
    return 0
