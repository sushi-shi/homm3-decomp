...
"""homm3.match.status - the objdiff scoreboard and checkpoint ledger.
  ...
  homm3 status check     report only newly changed functions whose score fell
                         from the preceding current checkpoint; unchanged
                         below-MAX rows are silent and MAX remains banked
...
def checkpoint_drops(current: dict, hashes: dict,
    ...
    A function that remains below MAX is queue state, not a fresh drop.  It is
    reported only when its own hash changed and its score fell from the prior
    current checkpoint.  MAX remains banked regardless.
    """
    drops = []
...
def cmd_check(report: dict) -> int:
    ...
    print(f"[status] no newly changed function regressed ({len(rows)} "
          "banked functions; unchanged below-MAX rows suppressed)")
    return 0
