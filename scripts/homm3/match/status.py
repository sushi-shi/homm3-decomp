...
"""homm3.match.status - the objdiff scoreboard and checkpoint ledger.
  ...
  homm3 status check     report when a function's source hash changed and its
                         new MAX falls below the preceding MAX; unrelated CUR
                         movement is silent and score drops are observational
...
from homm3.build.report import COMPARISON_POLICY
...
# current source implementation and comparison policy; hist_fuzzy = all-time
# high across implementations and policies. A proven src_hash change resets
# max_fuzzy to cur_fuzzy while hist_fuzzy retains the old peak. Unrelated CUR movement leaves MAX held.
# A comparison-policy change resets MAX to the next CUR, retaining old peaks
# only in HIST. Unobserved rows keep their identity with CUR unknown and MAX 0.
...
def load_baseline(path: Path | None = None) -> dict[tuple[str, str], MatchRow]:
    ...
    if path.is_file():
        ...
        policy_current = f"# comparison_policy: {COMPARISON_POLICY}" in lines
        for line in lines:
        ...
        if not policy_current:
            rows = {key: replace(row, cur=None, max=0.0,
                                 hist=max(row.hist, row.max, row.cur or 0.0))
...
def checkpoint_drops(current: dict, hashes: dict,
    ...
    for key in sorted(current.keys() | hashes.keys()):
        ...
        if (row.src_hash is None or src_hash is None
            ...
            continue
        if value is None or value < row.max - EPS:
            drops.append((key, row.max, row.hist, value))
    return drops
...
def cmd_check(report: dict, *, fingerprint_pair: tuple[dict, dict] | None = None) -> int:
    ...
    for (unit, fn), previous_max, historical, value in drops:
        ...
        print(f"[status] SOURCE-EDIT MAX DROP {unit} {fn}: "
              ...
              f"(HIST held at {historical:.2f}%)")
    if drops:
        print(f"[status] {len(drops)} changed function(s) reset MAX below "
