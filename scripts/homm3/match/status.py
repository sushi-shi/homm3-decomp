...
"""homm3.match.status - the objdiff scoreboard and checkpoint ledger.
  ...
  homm3 status check     report only a function whose own source changed while
                         it sat AT its banked MAX and then fell; unchanged rows
                         and holdouts (rows already below their banked MAX)
                         are never reported - we chase MAX, not cur
...
def checkpoint_drops(current: dict, hashes: dict,
    ...
    We chase MAX, not cur.  A row already below its banked MAX is a HOLDOUT:
    its movement is queue state, never a fresh drop, whether or not its source
    changed - no agent recovers what is already banked by hash, so nothing
    reports it.  A drop is reported only when the function sat AT its banked
    MAX, its own hash changed, and its score fell.  MAX remains banked
    regardless; nothing here is fatal.
    """
    drops = []
    for key, row in sorted(rows.items()):
        ...
        if (row.cur is None or row.src_hash is None or src_hash is None
            ...
            continue
        if row.cur < row.max - EPS:
            continue  # holdout below its banked MAX: never reported
...
def cmd_check(report: dict) -> int:
    ...
    print(f"[status] no newly changed function regressed ({len(rows)} "
          "banked functions; unchanged rows and holdouts below their banked "
          "MAX are never reported)")
    return 0
