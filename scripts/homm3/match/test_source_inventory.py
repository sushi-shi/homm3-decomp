...
class SourceInventoryTest(unittest.TestCase):
    ...
    def test_dc_inlined_helper_requires_its_reviewed_disposition(self):
        d = replace(definition('writeField'), va=None, inline=True)
        key = d.file, d.name, d.signature
        rows, errors = reconcile([d], [], {}, {},
                                 {key: 'DC caller scopes prove the expanded helper.'})
        self.assertEqual(errors, [])
        self.assertEqual(rows[0]['status'], 'documented_dc_inlined')
        self.assertEqual(rows[0]['reason'], 'DC caller scopes prove the expanded helper.')
        _, errors = reconcile([], [], {}, {}, {key: 'Stale'})
