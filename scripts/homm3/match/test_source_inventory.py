...
class SourceInventoryTest(unittest.TestCase):
    ...
    def test_audit_loads_the_unified_inferred_helper_disposition(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            (root / 'config/source/owner_placements.tsv').write_text(
                'file\tfunction\tsignature\tdc_file\treason\n')
            (root / 'config/source/order_placements.tsv').write_text(
                'file\tfunction\tsignature\tpreceding_function\tdc_file\treason\n')
            ...
            with patch.object(inventory, 'Project'), \
                    patch.object(inventory.ownership, 'collect', return_value=([d], [], {})), \
                    patch.object(inventory.ownership, 'active_stub_definitions', return_value=[]), \
                    patch.object(inventory.inputs, 'dreamcast_symbols',
