...
Two gates, each able to FAIL (the repo's negative-control rule):
...
def run_check(args) -> int:
    want = {g: getattr(args, g, False) for g in ("argv", "il", "inline", "reg")}
    ...
    # The behavioral gate is the HARD gate (it carries the oracle's bogus-EXPECT
    # negative control). Consistency is INFORMATIONAL: the solvers' masked
    # distance is a coarser proxy than objdiff's fuzzy, so exact-but-nonzero is
    # a known metric-skew (catalog C9), reported for health, not a build stop.
