...
def refresh_unit(unit: str, *, run=subprocess.run) -> str | None:
    from homm3.build.refresh import refresh_unit as refresh
    try:
        result = refresh(unit, run=run)
    except RuntimeError as exc:
        die(str(exc))
    did = [name for name, changed in (("compiled", result.compiled),
           ("normalized", result.normalized), ("report", result.report)) if changed]
    if not did:
        return None
    return (f"[refreshed {unit}: {' + '.join(did)} in {result.seconds:.1f}s; "
