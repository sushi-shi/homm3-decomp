...
# --- executable-wide tables -------------------------------------------------

def _hex(value: str) -> int:
    if not HEX_RE.match(value):
        raise AddressError(f"expected lowercase 0x hex, found {value!r}")
    return int(value, 16)


def read_functions(root: Path) -> dict[int, int]:
    path = root / FUNCTIONS_TSV
    if not path.is_file():
        return {}
    _banner, header, rows = read_tsv(path)
    if header != ["offset", "size"]:
        raise AddressError(f"{FUNCTIONS_TSV}: expected offset/size columns")
    spans: dict[int, int] = {}
    for row in rows:
        offset, size = _hex(row["offset"]), _hex(row["size"])
        if offset in spans:
            raise AddressError(f"{FUNCTIONS_TSV}: duplicate offset {offset:#x}")
        spans[offset] = size
    return spans


def read_names(root: Path, relative: str) -> list[tuple[int, str]]:
    path = root / relative
    if not path.is_file():
        return []
    _banner, header, rows = read_tsv(path)
    if header != ["offset", "name"]:
        raise AddressError(f"{relative}: expected offset/name columns")
    return [(_hex(row["offset"]), row["name"]) for row in rows]


def read_dispositions(root: Path) -> dict[str, tuple[str, str]]:
    path = root / DISPOSITIONS_TSV
    if not path.is_file():
        return {}
    _banner, header, rows = read_tsv(path)
    if header != ["identity", "disposition", "evidence"]:
        raise AddressError(f"{DISPOSITIONS_TSV}: expected identity/disposition/evidence columns")
    result = {}
    for row in rows:
        if row["disposition"] not in DISPOSITIONS:
            raise AddressError(f"{DISPOSITIONS_TSV}: unknown disposition {row['disposition']!r}")
        if not row["evidence"].strip():
            raise AddressError(f"{DISPOSITIONS_TSV}: {row['identity']} lacks evidence")
        if row["identity"] in result:
            raise AddressError(f"{DISPOSITIONS_TSV}: duplicate identity {row['identity']}")
        result[row["identity"]] = (row["disposition"], row["evidence"])
    return result


def _banner(kind: tuple[str, ...], pef_sha256: str, pef_size: int) -> list[str]:
    return [
        "# MANUALLY MANAGED - INITIALLY GENERATED by `homm3 mac migrate` from the",
        "# reviewed config/mac TOML inventories and hand-owned after admission.",
        *(f"# {line}" for line in kind),
...
def reviewed(root: Path) -> list[Reviewed]:
    ...
    runtime = tomllib.loads((root / "config/mac/runtime.toml").read_text())
    for row in runtime.get("functions", []):
...
def migrate(root: Path, pef) -> dict[str, int]:
    ...
    for row in rows:
        if spans.get(row.offset, row.size) != row.size:
            raise AddressError(f"{FUNCTIONS_TSV}: {row.offset:#x} has size {spans[row.offset]:#x}, "
                               f"reviewed {row.size:#x}")
        spans[row.offset] = row.size
    ...
    primary = dict(read_names(root, RUNTIME_MAP_TSV))
    aliases = set(read_names(root, RUNTIME_ALIASES_TSV))
    for row in rows:
        if row.kind != "runtime":
            continue
        if primary.get(row.offset, row.symbol) == row.symbol:
            primary[row.offset] = row.symbol
        else:
            aliases.add((row.offset, row.symbol))
    ...
    write_tsv(root / FUNCTIONS_TSV,
              _banner(("Every verified function span in the pinned PEF code section; names",
                       "come from source MAC_ADDRESS claims and the runtime maps."),
                      digest, length),
              ["offset", "size"], [[f"0x{o:x}", f"0x{s:x}"] for o, s in ordered])
    write_tsv(root / RUNTIME_MAP_TSV,
              _banner(("Library, runtime and compiler-support code: label only; the extent",
                       "is the functions.tsv row at the same offset. Not game functions."),
                      digest, length),
              ["offset", "name"], [[f"0x{o:x}", n] for o, n in sorted(primary.items())])
    write_tsv(root / RUNTIME_ALIASES_TSV,
              _banner(("Additional linkage names of one folded runtime body in runtime-map.tsv.",),
                      digest, length),
              ["offset", "name"], [[f"0x{o:x}", n] for o, n in sorted(aliases)])
...
def check(root: Path, pef, claims: list[Claim], windows: list[WindowsClaim]) -> list[str]:
    """Bounds, table resolution, target bytes and agreement with review rows."""
    problems = []
    ...
    try:
        spans = read_functions(root)
        runtime = read_names(root, RUNTIME_MAP_TSV)
        aliases = read_names(root, RUNTIME_ALIASES_TSV)
    ...
    for claim in claims:
        ...
        owners[claim.offset] = claim.identity
    runtime_offsets = Counter(offset for offset, _ in runtime)
    for offset, count in runtime_offsets.items():
        if count > 1:
            problems.append(f"{RUNTIME_MAP_TSV}: duplicate offset {offset:#x}; use {RUNTIME_ALIASES_TSV}")
    for offset, name in runtime:
        if offset not in spans:
            problems.append(f"{RUNTIME_MAP_TSV}: {name} at {offset:#x} has no {FUNCTIONS_TSV} row")
        if offset in owners:
            problems.append(f"{RUNTIME_MAP_TSV}: {name} at {offset:#x} is also source claim {owners[offset]}")
    for offset, name in aliases:
        if offset not in runtime_offsets:
            problems.append(f"{RUNTIME_ALIASES_TSV}: {name} at {offset:#x} has no runtime-map row")
    names = Counter(name for _, name in [*runtime, *aliases])
    for name, count in names.items():
        if count > 1:
            problems.append(f"runtime maps: duplicate name {name}")
    ...
    runtime_spans = {offset for offset, _ in runtime}
    runtime_names = {(offset, name) for offset, name in [*runtime, *aliases]}
...
def coverage(root: Path, pef, claims: list[Claim]) -> dict:
    ...
    runtime = {offset for offset, _ in read_names(root, RUNTIME_MAP_TSV)}
    counts, sizes = Counter(), Counter()
    for offset, size in spans.items():
        owner = "source" if offset in source else "runtime" if offset in runtime else "unowned"
        counts[owner] += 1
        sizes[owner] += size
