...
def parse_unit(path: Path):
    ...
    for line, code in zip(source.splitlines(), mask_lexical_noise(source).splitlines()):
        ...
        if m and origin:
            ...
            origin = None
            continue
        m = _DC_ONLY.match(code)
        if m and origin:
            entries.append((origin[0], origin[1], "DC_ONLY",
                            int(m.group(1), 16), int(m.group(2), 0)))
            origin = None
            continue
        # An origin belongs to its next declaration, including an unclaimed
        # inline body or prototype. Do not let that body's comment anchor a
        # later unrelated COMDAT. Comments/literals and preprocessor wrappers
        # do not consume the pending origin; templates may precede VA().
        if origin and not code.lstrip().startswith("#") and ("{" in code or ";" in code):
            origin = None
    return entries
...
def main(argv=None) -> int:
    ...
    (OUT / "README.md").write_text(
        ...
        "Own-source origin annotations bracket each unit's retail span. Header-origin "
        "functions are residents, not anchors. An origin expires at an intervening "
        "unclaimed declaration or body; it cannot attach to a later unrelated claim.\n\n"
        f"The current source supplies {len(ordered)} spanned units, "
        f"{sum(i['anchors'] for _, i in ordered):,} own-source anchors and "
        f"{sum(len(i['residents']) for _, i in ordered)} header-origin residents. "
        f"Alphabetical order: {alphabetical}. Span overlaps: {len(overlaps)}.\n\n"
        f"Within the bounded interval `0x{band_lo:x}..0x{band_hi:x}`, "
        f"{total:,} carved functions have a relation: {counts['in-span']:,} lie "
        f"inside spans and {counts['bracketed']:,} lie between them. "
        f"There are {len(attribution):,} unclaimed in-span functions.\n\n"
        "See `units.tsv`, `order.tsv`, `gaps.tsv`, `attribution.tsv` and "
        "`functions.tsv` for generated details. These are ownership leads, not "
        "byte-match verdicts or proof of whole-TU closure. Units without explicit "
        "source-origin annotations are not automatically spanned; no origin is "
        "invented for them. Functions outside the bounded interval require separate review.\n"
    )
    return 0
