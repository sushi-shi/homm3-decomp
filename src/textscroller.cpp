...
// Before normalization (type): type_text_slider.
class TextSlider : public Slider {
public:
    TextScroller* m_owner;  // +0x68

    TextSlider(int x, int y, int w, int h, int id, int num,
                     TSliderFunction func, Graphics graphics, int page,
                     unsigned char hotKey, TextScroller* scroller)
...
VA(0x005B9FB0, 0x2FF)
TextScroller::TextScroller(const char* text, int x, int y,
...
VA_COMPGEN(0x005BA2B0, 0x21, SCALAR_DELETING_DTOR, TextScroller)
...
VA(0x005BA2E0, 0xC6)
int TextScroller::open(int newPriority, HeroWindow* parent)
{
...
VA(0x005BA4C0, 0x13D)
int TextScroller::main(Message& msg)
{
        ...
        case WIDGET_CLEAR_STATUS:
            for (unsigned int i = 0; i < m_lineImages.size(); i++)
                m_lineImages[i]->sendMessage(
                    static_cast<Widget::Commands>(msg.m_codeX), msg.m_extra);
            if (m_textLines.size() > m_lineImages.size())
...
VA(0x005BA600, 0xD7)
void TextScroller::refresh(int firstLine)
{
...
void TextScroller::setText(const char* text)
{
    Font* textFont = ResourceManager::getFont(m_fontFilename);
    m_textLines.clear();
    textFont->fillLinesVector(text, m_width - 24, m_textLines);
