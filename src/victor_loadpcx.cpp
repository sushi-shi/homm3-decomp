...
// Residual 69.02%: all 15 real calls agree; the switch-table self-reference
// moves with the body. Four JSON-batched copy/validation scopes favor a
// shared copy arm and nested validation. Early exits with duplicated copy
// arms score 67.19%. Register/local allocation and refill scheduling remain.
                    ...
                    * image->m_buffwidth + (image->m_bmh->biBitCount * image->m_stx >> 3);
                    ...
                    while (rowsRemaining) {
                        ...
                        case victorPcxNibbleMode: {
                                ...
                                do {
                                    if (pixels & 1)
                                    decoded[pixels - 1] = *packed-- >> 4;
                                    else
                                    decoded[pixels - 1] = *packed & 15;
                                } while (--pixels);
                                goto copyRow;
                            }
