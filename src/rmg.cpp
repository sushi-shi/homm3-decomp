...
void type_random_map_generator::createRiver(TRmgMapPosition source)
{
    ...
    int mapItemCount = m_map.m_mapWidth * m_map.m_mapHeight * m_map.m_numberLevels;
    while (mapItemCount--) {
    ...
    while (!openPositions.empty()) {
        ...
        int positionCost = mapItem->m_movement.m_cost;
        direction = 0;
        TPoint* directionOffset = g_rmgDirections;
        for (; directionOffset < g_rmgDirections + 8;
             directionOffset += 2, direction += 2) {
            nextPosition = TRmgMapPosition(
                position.m_x + directionOffset->m_x,
                position.m_y + directionOffset->m_y,
                position.m_z);
            ...
            if (landType == eTerrainWater || landType == eTerrainRock
                || mapItem->m_tileData.m_impassable
            ...
            if (mapItem->m_tileData.m_riverTarget) {
                openPositions.clear();
                break;
            }
        }
    ...
    unsigned blockedDirections = mapItem->m_tileData.m_blockedDirections;
    if (blockedDirections) {
        ...
        for (; prototypeIndex < m_objectPrototypes[TERRAIN_RIVER_DELTA].size();
            ...
            if (properties->m_prototype->m_recommendedTerrainMask.test(landType)
                && deltaIndex-- == 0)
                break;
        }
        ...
        nextPosition = TRmgMapPosition(
            nextPosition.m_x + g_rmgDirections[direction * 2].m_x,
            nextPosition.m_y + g_rmgDirections[direction * 2].m_y,
            nextPosition.m_z);
