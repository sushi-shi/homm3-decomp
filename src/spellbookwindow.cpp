...
//
// Function coverage closed against pinned Complete: 0x59ba80..0x59dc64
// holds nine game bodies and the scalar deleting destructor, all exact.
// The native sort routines at 0x59dc90/0x59def0 and the shared string erase
// body at 0x41bdb0 are exact too. The reset origin annotation now includes
// it in the regenerated span instead of leaving it in the leading gap.
//
// Flanks: soundmgr ends with threadStopMP3 at 0x59b080..0x59b295. The
// twenty-two leading entries at 0x59b2a0..0x59ba20 are admitted cinit892..913
// (guard/atexit and terrain-bitset initialization). The trailing gap holds
// cinit914 at 0x59dc70 and the two native sort routines; spelldefs begins
// with SpellTargetsASingleArmy at 0x59e060. No unclaimed game slot remains.
//
// DC roster: ten of fifteen owning-cpp records survive as those retail
// bodies. get_level_string, DisplayNewSchool, convertID2HelpID and the two
// school-position conversions expand in the exact callers; their canonical
// definitions and source calls remain. Seven header methods/constructors
// also expand. The other 62 records belong to the replaced STLport library;
// twenty terrain initialization thunks are the excluded initialization class.
//
// Source-audit limits: std::string/SpellID use the existing type aliases;
// nested TSpellbookEntry spelling is the same class. The normalized school-
// position source calls are present despite the audit's missing-call leads.
// Its destructor-definition coverage gap and the shared virtual message*
// versus DC message& hierarchy debt remain explicit. Constructor terrain
// widening/cache-call differences and GotoPage's later mastery call are
// explained at their owning statements; operator< retains DC-public bool.
#include <va.h>
#include <algorithm>
#include <string.h>
#include <vector>
...
// E:\gamedcs\spellbookwindow.cpp:69
// The remembered-view reset starts this compiland's game-body span at
// 0x59ba80. game::ResetGame (0x4cecb0) is
...
void TSpellbookWindow::gotoPage(int page)
{
    ...
    // pointer-only 86.2235, by-value `entry` local 81.6941 (DC proves const&),
...
// DC public ??MTSpellbookEntry@TSpellbookWindow@@QBA_NABV01@@Z proves
// native bool despite the byte primitive in its formal debug record.
// Retail calls _strcmpi (the PC CRT spelling of DC stricmp).
