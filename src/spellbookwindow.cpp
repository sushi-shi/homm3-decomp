...
// Ordinary cpp helper: DC680 emits it before convertID2HelpID/WindowHandler.
// Complete expands its calls in the handler. Removing the unsupported inline
// keyword preserves every tracked byte in all seven header-consuming TUs.
void TSpellbookWindow::displayNewSchool(int position)
{
...
// The retail help table is in display order, not increasing widget-ID
// order: previous/next, adventure/combat, five schools, spell points, exit.
// Both inlined lookup pools (handler +0xabc and +0xb78) prove these values.
// The old 0..9 mapping by widget ID displayed the wrong right-click and
// rollover help despite a 99.9011% score and matching visible instructions.
// Nine complete helper variants reproduced their retained results. The
// correct values in help-row source order make the handler exact. A single
// assigned-result switch emits a different caller CFG (98.7794%); retaining
// the old case order after correcting values reaches 99.6439%. The negative
// guard is recorded at DC703/704; an extra upper-bound guard is byte-neutral.
// Preserve this ordinary helper and both source calls.
