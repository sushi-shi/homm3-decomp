...
// BLOCKED on both open decisions at once: the body opens an fs:[0]
// frame (P2.2, EH scopes wait on the synth-PDB) around a
// std::vector<army*> it fills with push_back and then hands to
// std::sort with the func_moves_before comparator (P2.3, the STLport
// vendoring decision). Everything else is decoded: the walk is
// armies[side][i] over both sides, each surviving stack gets a move
// key written to army+0x190 (-1000 - speed for a waiting stack,
// -speed when its side is not the AI's, +speed otherwise, and the two
// sentinels -0x2710 / -0x186a0 for war machines and the
// already-acted), after which a stable-partition pass swaps equal-key
// stacks so the acting side moves first.
