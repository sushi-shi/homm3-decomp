...
//
// Function coverage closed against the pinned Complete image: the fresh
// link-order span 0x423c80..0x4276bf contains 35 game bodies, all claimed
// and exact. The five retained Dinkumware routines at 0x4276c0, 0x427750,
// 0x427780, 0x427a90 and 0x427c30 are also exact. getFastestSpeed was
// merely absent from the generated coverage count because its origin
// comment carried trailing prose; its source-line annotation is now separate.
//
// Flanks: 0x4224e0..0x423887 contains six ai.cpp combatManager bodies and
// eight native STL/comparator routines, corroborated by the adjacent AI
// roster and their claimed calls. The other eleven leading-gap entries
// are cinit57..67. After the five retained STL routines, the twelve other
// trailing-gap entries are cinit68..79 (0x427a70 and 0x427ca0..0x428070),
// before ai_player's first body at 0x4280e0. Their admitted init-table
// membership and retail guard/atexit, bitset and global-table writes exclude
// game-method ownership; see config/retail-init-thunks.tsv.
//
// DC roster: 54 owning-cpp records resolve to 35 retained bodies, 17
// expanded helpers/special members and two absent retail bodies, documented
// at their inactive records below. The seven hero/AI header methods expand
// into callers (including the sort comparison); 67 STLport records are the
// older library surface, replaced by the pinned Dinkumware implementation.
// Twenty terrain-header initialization thunks are the excluded init class.
// The retained helper boundaries include the static doEagleEye and ordinary
// summoning helpers; their callers reproduce retail without inlining pins.
//
// Source-audit exceptions remain explicit: the shared SpellID declaration
// uses the repository's int ABI alias, while this function retains the
// original domain spelling; chooseMelee's decorated DC name proves bool
// despite its byte-storage debug primitive. PC bonus ordering and army-based
// necromancy differ from DC and are explained at the affected functions.
// These are reviewed source facts, not missing retail function bodies.
//
...
//
// Canonical getPrimarySkill's direct member expressions recover 92.7119%
// and the retained _Unguarded_partition call at retail +0x4e3. The separately
// claimed 100-byte helper now emits exact. Its prior signed-byte temporary
// made sort over-expand that site, leaving 83.4275% after std::vector
// ownership was recovered (older synthetic-vector HIST 91.9548%). The
// remaining register/stack mismatch closes with the speed and attribute
// bindings below. No suppression or private-field container adapter is used.
// DC283 captures speed in r10, DC284 captures attributes in r9, and later
// statements use both across the record stores and classifier call. Retail
// likewise computes speed and saves attributes before storing unit fields.
// Both source bindings restore its 0x198 frame and register assignment.
//
// A behavioral correction supplies the last operand: combatValuePerHit uses
// unit.value, not unit.totalValue. Retail loads unit+0x3c at 0x42439a;
// DC295 loads the same field via r11+36, with r11 at unit+24. Using the
// stack total wrongly multiplied this per-hit ratio by population. The
// 32-state record family emits eight objects, all reproduced: speed alone
// gives 96.2279%, attributes alone 94.4426%, both with the wrong numerator
// 99.9981%, and both with the per-creature numerator reach exact. Ratio
// placement and the enum-view spelling are byte-flat; retain DC295 before
// DC296..298's modifier defaults and the canonical armyTypes reader.
// The native numeric fixture checks 3584 record cases and rejects stack-
// total, wrong hitpoint denominator and missing population-factor controls.
...
// Before normalization (locals): dismiss_hero; the copied record is DC unit.
...
// Line 444 explicitly calls includes.h's
...
// Before normalization (locals): extra_targets, target_index.
