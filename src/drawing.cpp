...
// Function coverage closed: all 30 retained bodies at 0x4922f0..0x496c4d
// are exact, including the oldHexes static destructor. The rebuilt link-order
// map has 21 own-cpp anchors and no uncovered game function in that span.
//
// The 75-record DC roster comprises 43 owning-cpp records, 20 excluded
// terrain initializers and 12 header helpers. Of the owning-cpp records,
// 30 map to retained bodies and six expand at preserved source calls:
// get_creature_spell_message, UpdateCombatArea(extent), ScrollTo(extent),
// ScrollTo(rectangle), DrawObstacleAt and DrawDeadOccupants. The remaining
// seven (FullUpdate, rectangular UpdateCombatArea, ScrollCombatArea,
// positional ScrollTo, ScrollToPixel, DrawCreatureAlpha and Rescale) have
// no retained PC slot in the exhaustive bracket. The twelve header records
// are eleven expansions and the unused DrawCreatureAlpha bitmap facade.
//
// Flanks: cinit320..331 occupy 0x491ee0..0x492290 after dimensiondoorwindow;
// cinit332 at 0x496c50 and six CDPlay header/destructor bodies occupy the
// trailing gap before dxplay's 0x496d10. Their init-table entries, owning
// claims and CDPlay vtable slots account for the gaps. ds_engine is the
// Dreamcast sound backend, with no PC body competing for this bracket.
//
// Source boundaries remain canonical. A four-state keyword-removal family
// reproduces all 30 scores with ordinary DrawObstacleAt/DrawDeadOccupants.
// Public _N decorations recover the Boolean drawing/message interfaces;
// all 43 affected consumers retain their scores. Source-audit limits remain
// explicit: its lowered-byte type records conflict with those public types,
// static-member names produce unmatched call leads, and several expanded
// helpers have no retail claim for the audit to select.
#include <va.h>
#include <string.h>
#include "drawing.h"
#include "bitmap16.h"
...
// Recovering enclosing validity/type guards, ordinary target
// rejection returns and switch breaks removes all eleven former gotos.
...
// before restoring the two earlier GetName calls.
...
// DC line 237 calls GetName before the restrictions. The restored wrapper
// initially gave 88.2444%; the conditional return inside GetArmyName's else
// recovers all 1356 retail bytes and all 45 relocation positions. Flattening
// the two wrapper calls is unnecessary. The DC public symbol
// ?show_creature_spell_error@combatManager@@AAA_NPADPBVarmy@@@Z encodes
// bool despite its lowered T_UCHAR return record; keep that source interface.
...
// CodeView evidence. The ordinary declaration is sufficient: the four-state
// keyword-removal family reproduces every drawing score at 100% without
// either helper's unsupported explicit inline keyword.
...
// remaining_width, draw_x, draw_y. DC records wt_table as a const pointer
// to const TWallTraits; restoring its top-level const leaves every byte exact.
...
// DC records hex as const hexcell&; restoring that type leaves all 327 bytes
// exact. The former mutable pointer was an unnecessary source difference.
...
// The seven sprite/bitmap drawing wrappers below retain the recorded
// SLimitData Clip/Include/Intersects and sprite dimension-accessor calls.
// Their 16 boundary combinations preserve every tracked drawing score;
// copying these helpers into the callers was unnecessary. DC source-call
// anchors are1678/1716/1788,1806/1814/1817,1838/1843/1846,
// 1965/1970/1973 and1993/1998/2001.
// DC calls the const Bitmap16Bit sprite overloads, whose accessors and
// forwarding calls now remain in CSprite.h. The public _N parameter codes
// establish native bool for the original flip/alpha flags; Complete's extra
// archer colour-row selector retains its independently decoded byte type.
// E:\gamedcs\drawing.cpp:1661
...
// The DC public suffix H_N proves integer isFlipped and Boolean
// SaveBiggestExtent, despite the latter's lowered T_UCHAR formal record.
...
// DC public suffix HPAVarmy@@H_N proves the final native Boolean flag.
...
// DC public suffix HHH_N proves the final native Boolean flag.
