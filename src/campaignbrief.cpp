...
void TCampaignBrief::select(int which)
{
    ...
    if (!m_campaign->m_scenarios[which]->m_options->vslot2()) {
...
void TCampaignBrief::updateAllyEnemyFlags()
{
    g_campaignBriefPlayerSlot =
        m_campaign->m_scenarios[m_selectedScenario]->m_options->getPlayerPosition(
...
void TCampaignBrief::updateBonusIcons()
{
    ...
    if (scenario->m_options->vslot2() == ScenarioStartOptions::CHOICE_COUNT_PAIR) {
    ...
    for (i = 0; i < scenario->m_options->vslot2(); i++) {
        ...
        if (i == g_game->m_campaign.m_briefingChoice)
            ...
            m_startBonusBorders[i]->sendMessage(widget::WIDGET_CLEAR_STATUS,
                                                 4);
        const char* name = scenario->m_options->vslot3(&g_game->m_campaign, i);
        if (scenario->m_options->vslot1(i)) {
            ...
            m_spriteBonusImages[i]->setIconFrame(scenario->m_options->vslot4(i));
...
{
    return m_options->vslot6(campaign, option);
}
...
static int campaignBriefHandler(message& msg)
{
    ...
    case DIALOG_RETURN_OK: {
        ...
        int gamePos = brief->m_campaign->m_scenarios[selected]
                          ->m_options->getPlayerPosition(choice);
