...
{
    int artifact = heroes[iHero]->get_artifact(TArtifactSlot(slot)).artifactId;
    if (artifact == ARTIFACT_NONE)
    {
...
void swapManager::handle_artifact_click(long side, long id, unsigned char right_click)
{
    ...
    type_artifact old_artifact = our_hero->get_artifact(TArtifactSlot(slot));
...
int swapManager::Main(message& msg)
{
    ...
    if (gNetworkActive69954c)
    {
        ...
        if (static_cast<CSwapMgrNetMsgHandler*>(field_64)->m_field0c)
        {
            if (IsLeftHero())
            {
                CTradeRequestDoneMsg requestDone;
