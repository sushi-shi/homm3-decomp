#!/usr/bin/env python3
"""Generate the current VC6/Ninja object build from config/units.toml."""
...
def write_ninja(profiles: dict[str, list[str]], units: list[dict]) -> None:
    with (ROOT / "build.ninja").open("w") as stream:
        ...
        writer.comment("generated by configure.py from config/units.toml - do not edit")
        ...
        writer.build(
            "build.ninja",
            "configure",
            inputs=["configure.py", "config/units.toml"],
            implicit=["scripts/homm3/build/ninja_syntax.py"],
        )
        ...
        for unit in units:
            ...
            writer.build(
                ...
                "cl",
                inputs=unit["source"],
                implicit="scripts/homm3/build/cc_wrap.py",
                variables={
