...
**Executable matched: 96.43%** — fuzzy-weighted bytes over all 1,998,942 unfiltered bytes.

**Match score** — 4,085 / 4,764 functions exact (85.7%) across the full engine (4764 in linked units).

**Function exact MAX** — 4,108 / 4,764 current implementations (86.2%) have reached 100%.

| Module       | Units |     Functions exact |  Function exact MAX |   Fuzzy | Fuzzy Max |
| :----------- | ----: | ------------------: | ------------------: | ------: | --------: |
| `game`       |   138 | 4016 / 4695 (85.5%) | 4039 / 4695 (86.0%) |  96.39% |    96.62% |
...
- [ ] Review pragma debt in `src/`: **432 `#pragma` directives**, all for inlining
  (**211 `inline_depth(0)` and 5 `auto_inline(off)` regions**, including their resets).
