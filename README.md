...
_Generated by `homm3 build`; scores in the table are MAX, the best result for each function's current source._

**Executable MAX: 97.05%** — weighted by function size across 1,999,184 bytes of code included in matching.

**Function exact MAX** — 4,271 / 4,769 current implementations (89.6%) have reached 100%.

_CUR / MAX / HIST: 4,201 / 4,271 / 4,386 exact; 96.54% / 97.05% / 98.25% weighted fuzzy. CUR is this build; HIST retains peaks from earlier source implementations. Unchanged-source CUR dips do not lower MAX._

| Module       | Units | Functions exact MAX | Fuzzy MAX |
| :----------- | ----: | ------------------: | --------: |
| `game`       |   123 | 3593 / 3991 (90.0%) |    97.26% |
| `rmg`        |     3 |   301 / 368 (81.8%) |    94.37% |
| `network`    |     4 |   268 / 281 (95.4%) |    97.66% |
| `zlib-1.1.3` |    14 |    69 / 69 (100.0%) |   100.00% |
| `codec`      |     4 |     35 / 43 (81.4%) |    94.67% |
| `victor`     |     4 |      5 / 17 (29.4%) |    85.40% |
...
**Lightly optimized Classic Mac PowerPC reference (last full checkpoint):** 418 / 1019 scored functions exact; 59.28% of 228,048 compared bytes match. Scored functions are source claims whose full-TU body links; this is coverage, not the whole Mac game.
...
MAX to its new CUR. `homm3 status check` labels a byte-neutral reset separately
from a changed CUR; both are observational. Use
`homm3 status check --baseline-ref REF` to compare a lane with a committed
baseline before integration. A conflicted score ledger can be merged with
`homm3 status merge-baseline` and reviewed per row. Neither command changes
retail matching evidence.
