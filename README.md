...
**Match score** — 4,091 / 4,764 functions exact (85.9%) across the full engine (4764 in linked units).

**Function exact MAX** — 4,113 / 4,764 current implementations (86.3%) have reached 100%.

| Module       | Units |     Functions exact |  Function exact MAX |   Fuzzy | Fuzzy Max |
| :----------- | ----: | ------------------: | ------------------: | ------: | --------: |
| `game`       |   138 | 4022 / 4695 (85.7%) | 4044 / 4695 (86.1%) |  96.40% |    96.63% |
...
- [ ] Review pragma debt in `src/`: **418 `#pragma` directives**, all for inlining
  (**204 `inline_depth(0)` and 5 `auto_inline(off)` regions**, including their resets).
